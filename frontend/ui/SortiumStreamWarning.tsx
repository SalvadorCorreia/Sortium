import { useEffect, useState } from 'react';
import { Focusable, findModule } from '@steambrew/client';
import { queueService } from '../services/queue';

interface SortiumStreamWarningProps {
	activeMetric: string;
}

export function SortiumStreamWarning({ activeMetric }: SortiumStreamWarningProps) {
	const iconButtonModule = findModule((m) => m.IconButton) || {};
	const streamId = activeMetric.split('_')[0] || 'hltb';

	const [isRateLimited, setIsRateLimited] = useState<boolean>(queueService.getStreamState(streamId) === 'RATE_LIMITED');

	useEffect(() => {
		const unsubscribe = queueService.subscribe(() => {
			setIsRateLimited(queueService.getStreamState(streamId) === 'RATE_LIMITED');
		});
		return () => unsubscribe();
	}, [streamId]);

	if (!isRateLimited) {
		return null;
	}

	return (
		<div title="API Rate Limited - Stream suspended temporarily" style={{ display: 'flex' }}>
			<Focusable
				className={iconButtonModule.IconButton}
				style={{
					display: 'flex',
					alignItems: 'center',
					justifyContent: 'center',
					cursor: 'default',
					background: 'transparent',
					border: 'none',
					padding: '0 0.5em',
					marginLeft: '4px',
					fontSize: '1em',
					color: '#ffc82c',
				}}
				children={
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="none" width="1.2em" height="1.2em">
						<path
							fill="currentColor"
							fillRule="evenodd"
							clipRule="evenodd"
							d="M5.37033 3.07108C6.74074 2.15541 8.35191 1.66666 10.0001 1.66666C11.0944 1.66666 12.1781 1.88221 13.1891 2.301C14.2002 2.71979 15.1188 3.33362 15.8926 4.10744C16.6665 4.88126 17.2803 5.79992 17.6991 6.81097C18.1179 7.82202 18.3334 8.90565 18.3334 10C18.3334 11.6482 17.8447 13.2593 16.929 14.6297C16.0133 16.0002 14.7118 17.0683 13.1891 17.699C11.6664 18.3297 9.99084 18.4948 8.37433 18.1732C6.75782 17.8517 5.27297 17.058 4.10753 15.8926C2.94209 14.7271 2.14842 13.2423 1.82687 11.6258C1.50533 10.0092 1.67036 8.33369 2.30109 6.81097C2.93182 5.28825 3.99992 3.98676 5.37033 3.07108ZM12.2223 5L11.389 10H8.61119L7.77786 5H12.2223ZM11.9445 13.0556C11.9445 14.1294 11.074 15 10.0001 15C8.92619 15 8.05564 14.1294 8.05564 13.0556C8.05564 11.9817 8.92619 11.1111 10.0001 11.1111C11.074 11.1111 11.9445 11.9817 11.9445 13.0556Z"
						/>
					</svg>
				}
			/>
		</div>
	);
}
